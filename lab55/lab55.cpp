...
void Bfg9000_matrixX2(int** a, int num, int* versh, int size) {
	auto que = new int[size];
	for (int i = 0; i < size; i++)
	que[i] = 0;
	versh[num] = 1;
	int count = 0;
...
void Bfg9000_matrixX3(int** a, int num, int* versh, int size) {
	auto que = new int[size];
	for (int i = 0; i < size; i++)
		que[i] = 0;
	versh[num] = 1;
...
void main(){
	...
	system("pause");
	start = clock();
	Bfg9000_matrixX3(arr, num, versh, size);
	end = clock();
	cout << endl << endl << "   Время выполнения с очередью матрицей " << (double)difftime(end, start) / CLOCKS_PER_SEC << endl << endl << "   ";
